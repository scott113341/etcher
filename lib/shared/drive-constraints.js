...
const feedbackMessages = {
  ...
  TOOSMALL: 'Too Small For Image',
  LOCKED: 'Locked',
  SYSTEM: 'System Drive',
  SOURCE: 'Drive Contains Image'
};
 ...
 * });
 *
 * const { type, message } = DriveConstraints.getDriveFeedback(drive, image);
 *
 * if (type === 'warning') {
 *   // still allow flashing
 * } else if (type === 'error') {
 *   // disallow flashing
 * }
 */
exports.getDriveFeedback = (drive, image) => {

  // Mind the order of the if-statements if you modify.
  if (exports.isSourceDrive(drive, image)) {
    return {
      type: FEEDBACK_ERROR,
      message: feedbackMessages.SOURCE
    };

  } else if (exports.isSystemDrive(drive)) {
    return {
      type: FEEDBACK_ERROR,
      message: feedbackMessages.SYSTEM
    };

  } else if (!exports.isDriveLargeEnough(drive, image)) {
    return {
      type: FEEDBACK_ERROR,
      message: feedbackMessages.TOOSMALL
    };

  } else if (exports.isDriveLocked(drive)) {
    return {
      type: FEEDBACK_ERROR,
      message: feedbackMessages.LOCKED
    };

  } else if (!exports.isDriveSizeRecommended(drive, image)) {
    return {
      type: FEEDBACK_WARNING,
      message: feedbackMessages.NOTREC
    };
  }
